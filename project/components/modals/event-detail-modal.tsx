...
export default function EventDetailModal({
  ...
  const { isEventDetailModalOpen, closeEventDetailModal, selectedEvent, openEventDetailModal } =
    useUIStore();
  ...
  const projectName =
    userProjects.find((p) => p.id === selectedEvent.projectId)?.name || "Unknown Project";
  const creatorName = selectedEvent.creator
    ? `${selectedEvent.creator.firstName || ""} ${selectedEvent.creator.lastName || ""}`.trim() ||
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
      ...
      >
        <ConfirmActionModal
          ...
          title="Delete Event?"
          description={`Are you sure you want to cancel "${selectedEvent?.title}"? This action cannot be undone.`}
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-violet-50 flex-shrink-0">
          <div className="flex items-center gap-2 text-violet-700">
            {isEditing ? (
              <>
                <Edit2 size={20} />
                <span className="text-sm font-bold uppercase">Editing Event</span>
              </>
            ) : (
              <>
                <CalendarIcon size={20} />
                <span className="text-xs font-bold uppercase">{selectedEvent.type}</span>
              </>
            )}
          </div>
