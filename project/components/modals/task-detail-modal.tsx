...
export default function TaskDetailModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 dark:bg-black/80 backdrop-blur-sm px-4 py-8">
      <div className="bg-[#F8F8F8] dark:bg-zinc-950 rounded-[24px] shadow-2xl w-full max-w-5xl h-[95vh] md:h-[85vh] min-h-[500px] overflow-hidden animate-in fade-in zoom-in-95 duration-200 flex flex-col relative">
        {isLoading || !data ? (
          <div className="flex-1 flex flex-col items-center justify-center p-20">
            <Loader2 size={32} className="animate-spin text-gray-400 dark:text-zinc-500 mb-4" />
            <p className="text-gray-500 dark:text-zinc-400 font-medium">Loading task details...</p>
          </div>
        ) : (
          <TaskDetailContent
            key={selectedTaskId}
...
function TaskDetailContent({
  ...
  return (
    <>
      ...
      {/* HEADER */}
      <div className="flex items-center justify-between px-4 md:px-8 py-5 bg-white dark:bg-zinc-900 border-b border-gray-200 dark:border-zinc-800 flex-shrink-0">
        <div className="flex flex-col">
          <span className="text-xs font-bold text-gray-400 dark:text-zinc-500 uppercase mb-1">
            {data.project.name}
          </span>
          <input
            type="text"
            ...
            className="text-2xl font-bold text-black dark:text-zinc-100 bg-transparent border-none focus:outline-none focus:ring-0 p-0 placeholder:text-gray-300 dark:placeholder:text-zinc-600 w-full max-w-xl"
        ...
        <div className="flex items-center gap-3">
          {canDeleteTask && (
            <button
              ...
              className="p-2 text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-full transition-colors"
              title="Delete Task"
            >
              <Trash2 size={20} />
            </button>
          ...
          <button
            onClick={handleClose}
            className="p-2 text-gray-400 dark:text-zinc-400 hover:text-black dark:hover:text-zinc-100 hover:bg-gray-100 dark:hover:bg-zinc-800 rounded-full transition-colors"
          >
            <X size={24} />
          </button>
        </div>
      ...
      <div className="flex flex-1 overflow-y-auto md:overflow-hidden flex-col md:flex-row">
        ...
        <div className="w-full md:w-3/5 p-4 md:p-8 overflow-visible md:overflow-y-auto border-b md:border-b-0 md:border-r border-gray-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 shrink-0">
          {error && (
            <div className="mb-6 p-4 bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400 rounded-xl border border-red-100 dark:border-red-500/20 text-sm">
              {error}
            </div>
          )}
          ...
          <form id="edit-task-form" onSubmit={handleSaveChanges} className="space-y-6">
            <div>
              <div>
                <label className="block text-xs font-bold text-gray-700 dark:text-zinc-300 mb-2 uppercase">
                ...
                {canEditTask ? (
                  ...
                  <div
                    className="text-sm p-4 bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl [&_ul]:list-disc [&_ul]:pl-4 [&_ol]:list-decimal [&_ol]:pl-4 space-y-2 text-black dark:text-zinc-100"
                    dangerouslySetInnerHTML={{
                      __html:
                        contentHtml ||
                        "<p class='text-gray-400 dark:text-zinc-500 italic'>No details provided.</p>",
                    }}
                  />
                )}
              </div>
              ...
              <div>
                <label className="text-xs font-bold text-gray-700 dark:text-zinc-300 mt-2 mb-2 uppercase flex items-center gap-2">
                ...
                {selectedFile ? (
                  ...
                  <div className="flex items-center justify-between p-3 bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl group">
                    <a
                      href={attachmentUrl}
                      target="_blank"
                      rel="noreferrer"
                      className="flex items-center gap-2 text-sm font-medium text-blue-600 dark:text-blue-400 hover:underline truncate"
                    >
                    ...
                    {canEditTask && (
                      <button
                        ...
                        className="text-gray-400 hover:text-red-500 dark:text-zinc-500 dark:hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0"
                      >
                        <Trash2 size={16} />
                      </button>
                    )}
                  ...
                  <div className="text-sm text-gray-400 dark:text-zinc-500 italic p-3 bg-gray-50 dark:bg-zinc-900 rounded-xl border border-gray-100 dark:border-zinc-800">
                    No file attached.
                  </div>
                )}
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-4 border-t border-gray-100 dark:border-zinc-800">
              <div>
                <label className="text-xs font-bold text-gray-700 dark:text-zinc-300 mb-2 uppercase flex items-center gap-2">
                  <LayoutList size={14} /> Status
                </label>
                <select
                  value={statusId}
                  onChange={(e) => setStatusId(e.target.value)}
                  className="w-full px-4 py-3 bg-gray-50 dark:bg-zinc-950 border border-gray-200 dark:border-zinc-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-black dark:focus:ring-zinc-600 transition-all text-sm text-black dark:text-zinc-100 appearance-none cursor-pointer"
                >
            ...
            {canEditTask && (
              <div className="pt-6 flex justify-end">
                <button
                  ...
                  className="flex items-center gap-2 bg-black dark:bg-zinc-100 text-white dark:text-zinc-900 px-8 py-3 rounded-full text-sm font-bold hover:bg-gray-800 dark:hover:bg-zinc-300 transition-all disabled:opacity-50"
                >
                  {isUploading
                    ? "Uploading File..."
        ...
        <div className="w-full md:w-2/5 flex flex-col bg-[#F8F8F8] dark:bg-zinc-950 h-[600px] md:h-full shrink-0">
          <div className="px-6 py-4 border-b border-gray-200 dark:border-zinc-800 flex-shrink-0 bg-white dark:bg-zinc-900">
            <h3 className="font-bold text-black dark:text-zinc-100 flex items-center gap-2">
                ...
                if (item.feedType === "comment") {
                  ...
                  return (
                    <div key={`comment-${item.id}`} className="flex gap-3">
                      <div className="w-8 h-8 rounded-full bg-gray-300 dark:bg-zinc-800 text-gray-600 dark:text-zinc-300 flex items-center justify-center font-bold text-xs flex-shrink-0">
                      ...
                      <div className="flex flex-col flex-1">
                        <div className="flex items-baseline justify-between mb-1">
                          <div className="flex items-center gap-2">
                            ...
                            {item.isEdited && !isEditing && (
                              <span className="text-[10px] text-gray-400 dark:text-zinc-400 font-medium bg-gray-100 dark:bg-zinc-800 px-1.5 py-0.5 rounded-md">
                                (edited)
                              </span>
                            )}
                          </div>
                          ...
                          {isMyComment && !isEditing && (
                            <button
                              ...
                              className="text-gray-400 dark:text-zinc-500 hover:text-black dark:hover:text-zinc-200 transition-colors"
                            >
                              <Edit2 size={12} />
                            </button>
                          )}
                        ...
                        {isEditing ? (
                          <div className="mt-1 space-y-2">
                            ...
                            <div className="flex gap-2 justify-end">
                              <button
                                ...
                                className="text-xs font-bold text-gray-500 dark:text-zinc-400 hover:text-black dark:hover:text-zinc-100 px-3 py-1.5"
                              >
                                Cancel
                              </button>
                              <button
                                onClick={() => handleEditSubmit(item.id)}
                                className="text-xs font-bold text-white dark:text-zinc-900 bg-black dark:bg-zinc-100 hover:bg-gray-800 dark:hover:bg-zinc-300 rounded-md px-3 py-1.5"
                              >
                                Save
                              </button>
                            </div>
                          ...
                          <div
                            className="bg-white dark:bg-zinc-900 p-3 rounded-xl rounded-tl-none border border-gray-200 dark:border-zinc-800 shadow-sm text-sm text-black dark:text-zinc-100 [&_strong]:text-black dark:[&_strong]:text-zinc-100 [&_strong]:font-bold [&_p]:m-0"
                      ...
                      <div className="text-sm text-gray-600 dark:text-zinc-400">
                        ...
                        {item.actionType === "updated_priority" && (
                          <>
                            ...
                            <span className="font-medium text-black dark:text-zinc-100 capitalize">
                              {item.oldValue}
                            </span>{" "}
                            to{" "}
                            <span className="font-medium text-black dark:text-zinc-100 capitalize">
          ...
          <div className="mt-auto p-4 bg-white dark:bg-zinc-900 border-t border-gray-200 dark:border-zinc-800 flex-shrink-0">
            <form onSubmit={handleCommentSubmit} className="flex flex-col gap-2">
              ...
              <div className="flex justify-end">
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-black dark:bg-zinc-100 text-white dark:text-zinc-900 rounded-2xl text-sm font-bold hover:bg-gray-800 dark:hover:bg-zinc-300 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isSubmittingComment ? (
