...
// GLOBAL ACTIONS (Requires System Permissions)
...
export async function removeTeamMemberGlobalAction(memberId: string) {
  try {
    ...
    if (!canManageTeam) return { success: false, error: "Access Denied: You cannot manage global team members." };
...
export async function inviteUserByEmailAction(projectId: string, email: string) {
  try {
    ...
    if (!canInvite) return { success: false, error: "Access Denied: You cannot invite external members." };
    ...
    if (!isOwner && localRole !== "admin") {
      return { success: false, error: "Access Denied: You must be a Project Admin to send invites." };
    }
...
// LOCAL ACTIONS (Requires Project Admin/Owner)
...
export async function inviteMembersAction(projectId: string, memberIds: string[]) {
  try {
    ...
    if (!isOwner && localRole !== "admin") {
      return { success: false, error: "Access Denied: Only Admins can invite members to this project." };
    }
...
export async function updateProjectDetailsAction(projectId: string, data: { name: string; description: string; dueDate: string | null }) {
  try {
    ...
    if (!isOwner && localRole !== "admin") {
      return { success: false, error: "Access Denied: You must be a Project Admin or Owner to edit project details." };
    }
...
export async function markProjectCompletedAction(projectId: string) {
  try {
    ...
    if (!isOwner && localRole !== "admin") {
      return { success: false, error: "Access Denied: You must be a Project Admin or Owner to complete the project." };
    }
