...
import OnboardingModal from "@/components/modals/onboarding-modal"; // <-- Import the modal
...
export default async function DashboardLayout({ children }: { children: ReactNode }) {
  ...
  if (clerkUser) {
    dbUser = await queries.users.getById(clerkUser.id);
    userRoleName = await queries.users.getRoleName(clerkUser.id);
    assignableRoles = await queries.users.getAssignableRoles();
  }
